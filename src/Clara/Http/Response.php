...
class Response extends Message implements Stringable {
	...
	public function setStatusCode($code) {
		if( ! is_numeric($code)) {
			throw new InvalidArgumentException('Invalid HTTP response code ' . $code . ' given to ' . __METHOD__);
		}
		$code = (int) $code;
		if($code < 100 || $code >= 600 || ! array_key_exists($code, self::$statusTexts)) {
			throw new OutOfRangeException('Invalid HTTP response code ' . $code . ' given to ' . __METHOD__);
		}
	...
	public function sendHeaders() {
		...
		//status first
		header(sprintf('%s %s %s', $this->protocol, $this->statusCode, $this->getStatusText()));
